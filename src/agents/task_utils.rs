use crate::models::{AgentType, Priority, Task, TaskExecutionResult, TaskResult};
...
pub fn create_success_result(
    ...
    metadata.insert(
        "files_created_count".to_string(),
        files_created.len().to_string(),
    );
    metadata.insert(
        "files_modified_count".to_string(),
        files_modified.len().to_string(),
    );
...
pub fn create_failure_result(
    ...
    metadata.insert(
        "has_partial_output".to_string(),
        partial_output.is_some().to_string(),
    );
...
pub fn estimate_task_complexity(
    content: &str,
...
mod tests {
    ...
    fn test_priority_analysis() {
        assert_eq!(
            analyze_task_priority("critical security fix", &HashMap::new()),
            Priority::High
        );
        assert_eq!(
            analyze_task_priority("add new feature", &HashMap::new()),
            Priority::Medium
        );
        assert_eq!(
            analyze_task_priority("simple documentation update", &HashMap::new()),
            Priority::Medium
        );
    }
    ...
    fn test_complexity_estimation() {
        let (minutes, factors) =
            estimate_task_complexity("create api endpoint with database", &HashMap::new());
