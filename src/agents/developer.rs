...
use super::task_utils::{build_enriched_context, create_success_result, create_failure_result};
...
impl SoftwareDeveloperAgent {
    ...
    async fn detect_language_from_task(&self, task: &Task) -> Result<String> {
        ...
        let has_file_context = task.context.get("file_path").is_some();
        let has_project_context = task.context.get("project_type").is_some();
        let content_has_indicators = !task.content.to_lowercase().eq(&task.content); // Has language keywords
        
        if local_detected != "rust" || has_file_context || has_project_context || content_has_indicators {
        ...
        let context = task.context.get("project_type")
        ...
        if code_snippet.len() > crate::constants::CODE_SNIPPET_TRUNCATION_LENGTH {
            code_snippet = code_snippet.chars().take(crate::constants::CODE_SNIPPET_TRUNCATION_LENGTH).collect();
        }

        let detected = self.claude_client.detect_language(&code_snippet, context).await?;
    ...
    async fn build_code_generation_request(&self, task: &Task) -> Result<CodeGenerationRequest> {
        ...
        let context = build_enriched_context(
            ...
            Some({
                ...
                extra.insert("requirements_count".to_string(), requirements.len().to_string());
                extra
            })
        );
    ...
    fn create_success_result(&self, task: &Task, code_result: crate::claude_code::CodeGenerationResult) -> TaskResult {
        let files_created = code_result.files_to_create.iter()
            .map(|f| f.path.clone())
            .collect();
        
        let files_modified = code_result.files_to_modify.iter()
            .map(|f| f.path.clone())
            .collect();
        ...
        metadata.insert("code_length".to_string(), code_result.code.len().to_string());
        metadata.insert("explanation_length".to_string(), code_result.explanation.len().to_string());
        metadata.insert("has_tests".to_string(), code_result.code.to_lowercase().contains("test").to_string());
    ...
    fn create_failure_result(&self, task: &Task, error: &SpiralError) -> TaskResult {
        ...
        let mut metadata = HashMap::new();
        metadata.insert("agent_capability".to_string(), "code_generation".to_string());
...
impl Agent for SoftwareDeveloperAgent {
    ...
    async fn execute(&self, task: Task) -> Result<TaskResult> {
        ...
        if self.status.is_busy {
            ...
            return Ok(self.create_failure_result(&task, &SpiralError::Agent {
        ...
        debug!("Generated code request for language: {}", code_request.language);

        match self.claude_client.generate_code(code_request).await {
            Ok(code_result) => {
                let execution_time = start_time.elapsed().as_secs_f64();
                info!("Successfully generated code for task: {} in {:.2}s", task.id, execution_time);
    ...
    async fn analyze_task(&self, task: &Task) -> Result<TaskAnalysis> {
        ...
        let context = build_enriched_context(
            ...
            Some({
                let mut extra = HashMap::new();
                extra.insert("analysis_type".to_string(), "development_planning".to_string());
                extra.insert("capability_focus".to_string(), "code_generation".to_string());
                extra
            })
        );

        self.claude_client.analyze_task(&task.content, context).await
    }
}