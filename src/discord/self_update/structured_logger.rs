...
use crate::{Result, error::SpiralError};
use chrono::Utc;
use std::fs;
use std::io::Write;
...
impl StructuredLogger {
    ...
    pub fn new(update_id: String, codename: String) -> Result<Self> {
        ...
        fs::create_dir_all(&log_dir)
            .map_err(|e| SpiralError::SystemError(format!("Failed to create log directory: {}", e)))?;
        
        // Create main log file
        let main_log_path = log_dir.join("main.log");
        let main_log_file = fs::File::create(&main_log_path)
            .map_err(|e| SpiralError::SystemError(format!("Failed to create main log file: {}", e)))?;
        ...
        if let Ok(mut guard) = self.main_log.try_write() {
            if let Some(ref mut file) = *guard {
                file.write_all(formatted.as_bytes())
                    .map_err(|e| SpiralError::SystemError(format!("Failed to write to main log: {}", e)))?;
                file.flush()
                    .map_err(|e| SpiralError::SystemError(format!("Failed to flush main log: {}", e)))?;
            }
        }
    ...
    pub async fn log_to_phase(&mut self, phase: &str, message: &str) -> Result<()> {
        ...
        if !phase_logs.contains_key(phase) {
            ...
            let phase_file = fs::File::create(phase_path)
                .map_err(|e| SpiralError::SystemError(format!("Failed to create phase log: {}", e)))?;
            ...
            if let Some(file) = phase_logs.get_mut(phase) {
                ...
                file.write_all(header.as_bytes())
                    .map_err(|e| SpiralError::SystemError(format!("Failed to write phase header: {}", e)))?;
            }
        }
        ...
        if let Some(file) = phase_logs.get_mut(phase) {
            file.write_all(formatted.as_bytes())
                .map_err(|e| SpiralError::SystemError(format!("Failed to write to phase log: {}", e)))?;
            file.flush()
    ...
    pub async fn log_error(&mut self, phase: &str, error: &str, context: Option<&str>) -> Result<()> {
    ...
    pub async fn log_validation_results(&mut self, phase: &str, results: &str) -> Result<()> {
        ...
        let mut validation_file = fs::File::create(validation_path)
            .map_err(|e| SpiralError::SystemError(format!("Failed to create validation log: {}", e)))?;
        ...
        validation_file.write_all(header.as_bytes())
            .map_err(|e| SpiralError::SystemError(format!("Failed to write validation header: {}", e)))?;
        validation_file.write_all(results.as_bytes())
            .map_err(|e| SpiralError::SystemError(format!("Failed to write validation results: {}", e)))?;
        validation_file.flush()
            .map_err(|e| SpiralError::SystemError(format!("Failed to flush validation log: {}", e)))?;
    ...
    pub async fn create_summary(&mut self, success: bool, message: &str, duration: std::time::Duration) -> Result<()> {
    ...
    pub async fn archive_logs(&self) -> Result<PathBuf> {
        ...
        let archive_name = format!("{}.tar.gz", self.log_dir.file_name().unwrap().to_string_lossy());
        let archive_path = PathBuf::from(UPDATE_LOGS_DIR).join("archives").join(archive_name);
        
        // Create archives directory if it doesn't exist
        fs::create_dir_all(archive_path.parent().unwrap())
            .map_err(|e| SpiralError::SystemError(format!("Failed to create archives directory: {}", e)))?;
        ...
        tar.append_dir_all(
            self.log_dir.file_name().unwrap(),
            &self.log_dir
        ).map_err(|e| SpiralError::SystemError(format!("Failed to add files to archive: {}", e)))?;
