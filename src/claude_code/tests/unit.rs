...
fn test_prompt_injection_prevention() {
    ...
    for malicious_input in malicious_inputs {
        ...
        assert!(result.is_err(), "Should reject prompt injection: {}", malicious_input);
        
        if let Err(SpiralError::Validation(msg)) = result {
            assert!(msg.contains("Invalid request content"), 
...
fn test_prompt_construction() {
    ...
    let request = CodeGenerationRequest {
        ...
        context: {
            ...
            ctx.insert("coding_standards".to_string(), "SOLID principles".to_string());
            ctx
        },
    ...
    assert!(system_prompt.contains("security"), "Should include security guidelines");
    assert!(system_prompt.contains("quality"), "Should include quality standards");
    
    // ✅ USER PROMPT VALIDATION: Should include all request components  
    assert!(user_prompt.contains("Create a simple function"), "Should include description");
    assert!(user_prompt.contains("library"), "Should include context");
    assert!(user_prompt.contains("unit tests"), "Should include requirements");
    assert!(user_prompt.contains("Existing code context"), "Should include existing code");
    
    // 🛡️ SECURITY VALIDATION: Should not contain raw user input without sanitization
    assert!(!user_prompt.contains("{{"), "Should not have template injection vulnerabilities");
    assert!(!user_prompt.contains("}}"), "Should not have template injection vulnerabilities");
}
...
fn test_api_url_validation() {
    ...
    for config in invalid_configs {
        ...
        let result = ClaudeCodeClient::new(config);
        assert!(result.is_err(), "Should reject invalid/malicious URLs: {}", base_url);
    }
}
...
fn test_response_parsing() {
    ...
    let response: ClaudeCodeResponse = serde_json::from_str(valid_response_json)
        .expect("Should parse valid Claude response");
    
    // ✅ RESPONSE VALIDATION: Should extract expected fields
    assert_eq!(response.content.len(), 1, "Should have one content block");
    assert_eq!(response.content[0].content_type, "text", "Should be text content");
    assert!(response.content[0].text.is_some(), "Should have text content");
    ...
    let result: std::result::Result<ClaudeCodeResponse, _> = serde_json::from_str(invalid_response_json);
